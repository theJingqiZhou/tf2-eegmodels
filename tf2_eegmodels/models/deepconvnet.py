...
def DeepConvNet(
    ...
    x = layers.Conv2D(
        25, (1, 5), kernel_constraint=constraints.max_norm(2.0, axis=(0, 1, 2))
    )(input)
    x = layers.Conv2D(
        25,
        (channels, 1),
    ...
    x = layers.Conv2D(
        50, (1, 5), kernel_constraint=constraints.max_norm(2.0, axis=(0, 1, 2))
    )(x)
    x = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)(x)
    x = layers.ELU()(x)
    x = layers.MaxPooling2D(pool_size=(1, 2), strides=(1, 2))(x)
    ...
    x = layers.Conv2D(
        100, (1, 5), kernel_constraint=constraints.max_norm(2.0, axis=(0, 1, 2))
    )(x)
    x = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)(x)
    x = layers.ELU()(x)
    x = layers.MaxPooling2D(pool_size=(1, 2), strides=(1, 2))(x)
    ...
    x = layers.Conv2D(
        200, (1, 5), kernel_constraint=constraints.max_norm(2.0, axis=(0, 1, 2))
    )(x)
    x = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)(x)
    x = layers.ELU()(x)
    x = layers.MaxPooling2D(pool_size=(1, 2), strides=(1, 2))(x)
